...
export function FormFields({ mentor }: FormFieldsProps) {
  ...
  const selectedCount = specialties.length ?? mentor.specialties;

  const toggleSpecialty = (value: string): void => {
    if (specialties.includes(value)) {
      const next = specialties.filter(item => item !== value);
      setSpecialties(next);
      formik.setFieldValue('specialties', next);
  ...
  useEffect(() => {
    setOriginalImage(mentor.profile ?? '');
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  useEffect(() => {
    const len = formik.values.aboutMe?.length ?? mentor.aboutMe?.length ?? 0;
    setIsMaxCharactersExceeded(len > 600);
  }, [formik.values.aboutMe, mentor.aboutMe]);

  const handleMaxCharacters = () => {
    const len = formik.values.aboutMe?.length ?? mentor.aboutMe?.length ?? 0;
    if (len <= 600) return `${len} / 600`;
    return 600 - len;
  };
  ...
  return (
    <div className="grid grid-cols-[8rem_1fr] items-center gap-y-12 gap-x-16">
      <Modal.Root>
        <Modal.Control asChild>
          ...
          >
            ...
            <svg
              ...
              viewBox="0 0 40 40"
              fill="none"
              className="camera-icon text-gray-700 w-10 h-10 absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-[1]"
            >
              <path
        ...
        <EditPhotoModal
          selectedPhoto={formik.values.profile ?? mentor.profile ?? ''}
          onAddPhoto={photo => formik.setFieldValue('profile', photo)}
      ...
      <div>
        ...
        <div className="grid grid-cols-[repeat(3,9.5rem)] gap-4 mt-4">
          {specialtiesOptions.map(specialty => (
            <div
              key={specialty}
              className={`flex items-center justify-center gap-0.5 py-2 px-3 h-[2.2rem] rounded-[50px] text-center cursor-pointer [&_svg]:${formik.values.specialties?.includes(specialty) ?? mentor.specialties.includes(specialty) ? 'block' : 'hidden'} ${
                formik.values.specialties?.includes(specialty) ??
                mentor.specialties.includes(specialty)
                  ? 'bg-blue-600 text-white'
                  : 'bg-gray-350 text-gray-700'
              }`}
      ...
      <div aria-hidden />
      <section className="[&_label_&_span:first-child]:text-black-200 [&_label_&_span:first-child_.asterisk]:text-blue-700 [&_p]:text-right [&_p]:mt-2">
        <InputForm
        ...
        <p
          className={`text-[0.875rem] leading-4 ${isMaxCharactersExceeded ? 'text-red-400' : 'text-black-200'}`}
        >
          {handleMaxCharacters()}
        </p>
      </section>
